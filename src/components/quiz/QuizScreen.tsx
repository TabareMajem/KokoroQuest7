...
export default function QuizScreen({ assessmentType, onBack, onComplete }: Props) {
  ...
  const questions = assessmentType === 'ei' ? eiQuestions : bigFiveQuestions;
  ...
  const handleComplete = async () => {
    try {
      ...
      if (assessmentType === 'ei') {
        const quizResults = Object.entries(answers).map(([questionId, answerId]) => ({
          questionId,
          selectedAnswerId: answerId as string
        }));
        processedResults = await processEIResults(quizResults);
      } else {
        const responses = Object.entries(answers).map(([questionId, value]) => ({
          questionId,
          value: value as number
        }));
        processedResults = await processBigFiveResults(responses);
      }
